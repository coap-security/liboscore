...
/** @brief Set autogenerated options on a message up to and including a given number
 *
 ...
 * @return OK if the option could be added, any error depending on the failure cause
 *
 * This starts the generation of (outer or inner) options in a message up to
 * the a given point, and is typically called when a library user appends a
 * higher option then the last written one. In particular, this generates
 *
 * * the OSCORE option
 * * possibly the inner Observe option
 * * possibly inner Echo options used in the Appendix B.1 processing
 * */
OSCORE_NONNULL
oscore_msgerr_protected_t flush_autooptions_until(oscore_msg_protected_t *msg, uint16_t optnum)
{
        ...
        )
{
    ...
    *protected = unprotected->backend;
    ...
    // Before doing anything with lengths and sizes (and even before clearing
    // out is_first_use flags as they're still used to determine whether we're
    // reusing the request nonce inside there), make sure to flush out all
    // pending options (especially the OSCORE option)
    oscore_msgerr_protected_t opterr = flush_autooptions_until(unprotected, OPTNUM_MAX);
    if (oscore_msgerr_protected_is_error(opterr)) {
        return OSCORE_FINISH_ERROR_OPTIONS;
    }
